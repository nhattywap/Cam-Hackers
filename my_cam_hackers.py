...
class CamHacker:
    ...
    country_codes = ['US', 'JP', 'IT', 'KR', 'FR', 'DE', 'TW', 'RU', 'GB', 'NL', 'CZ', 'TR', 'AT', 'CH', 'ES', 'CA', 'SE', 'IL', 'PL', 'IR', 'NO', 'RO', 'IN', 'VN', 'BE', 'BR', 'BG', 'ID', 'DK', 'AR', 'MX', 'FI', 'CN', 'CL', 'ZA', 'SK', 'HU', 'IE', 'EG', 'TH', 'UA', 'RS', 'HK', 'GR', 'PT', 'LV', 'SG', 'IS', 'MY', 'CO', 'TN', 'EE', 'DO', 'SI', 'EC', 'LT', 'PS', 'NZ', 'BD', 'PA', 'MD', 'NI', 'MT', 'IT', 'SA', 'HR', 'CY', 'PK', 'AE', 'KZ', 'KW', 'VE', 'GE', 'ME', 'SV', 'LU', 'CW', 'PR', 'CR', 'BY', 'AL', 'LI', 'BA', 'PY', 'PH', 'FO', 'GT', 'NP', 'PE', 'UY', '-']
    page_num = [720, 232, 159, 141, 120, 107, 92, 82, 81, 66, 58, 54, 48, 44, 39, 38, 35, 31, 30, 22, 29, 28, 26, 23, 23, 23, 21, 16, 16, 13, 13, 23, 13, 13, 11, 11, 11, 11, 10, 11, 10, 10, 7, 8, 7, 6, 7, 7, 6, 6, 6, 6, 5, 6, 5, 5, 4, 5, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 16]
    ...
    def get_cam_ip(self):
        if self.num == '':
            return 'Interger value is None'
        for page, code in zip(self.page_num, self.country_codes):
            if self.num == self.country_codes.index(code) + 1:
                self.ip_getter(page, code)
            else:
                pass

    def ip_getter(self, pages, code):
        print("\n")
        try:
            headers = {'User-Agent': 'Mozilla/5.0 (X11; Linux i686; rv:68.0) Gecko/20100101 Firefox/68.0'}
            for j in range(pages):
                url = ("https://www.insecam.org/en/bycountry/%s/?page=%s" %(code, str(j)))
            ...
            print("Request Faild chekck your connection and try again")


if __name__=='__main__':
    while True:
        try:
            num = int(input("[0] to stop | OPTIONS : "))
            if num == 0:
                break
            if num > 91:
                print('WARRNINNG: wrong number of index')
                continue
            cam = CamHacker(num)
            cam.get_cam_ip()
